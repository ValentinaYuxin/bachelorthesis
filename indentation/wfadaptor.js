...
function WfIllustrator(wf_adaptor) { // View  {{{
  ...
  var draw_symbol = this.draw.draw_symbol = function (columnWidths, sname, id, title, row, col, group, addition) { // {{{
    ...
    let wshift = 0;
    let width = columnWidths[col]?.[row]?.width || 0;
    ...
    if (col === 1) {
      ...
      const columnwidth = columnWidths[col][row].minblock === 0
        ? (columnWidths[col][row].maxBlockHistory.length > 2
          ...
          : columnWidths[col][row].maxBlockHistory[0])
    ...
    if (parallelElement.length && columnWidths[col + 1] && columnWidths[col + 1][row] !== undefined) {
      parallelElement.attr('transform', 'translate(' + (columnWidths[col + 1][row].minblock / 2+columnWidths[col][row].block / 2)+ ', 0)');
    }
  ...
  var draw_border = this.draw.draw_border = function (columnWidths, id, p1, p2, group) { // {{{范围的虚线边框
    ...
    if (p1.col === 1) {
      ...
      // 遍历 2 到 p1.col - 1 这些列的 block 值总和
      ...
      let current = columnWidths[p1.col]?.[p1.row]?.minblock === 0
        ? (columnWidths[p1.col][p1.row].maxBlockHistory.length > 2
          ? columnWidths[p1.col][p1.row].maxBlockHistory[1] 
          : columnWidths[p1.col][p1.row].maxBlockHistory[0]) / 2
        : 0;
    ...
    if (columnWidths[p1.col][p1.row].minblock === 0) {
      ...
      const currentColWidth =
        currentCell.minblock === 0
          ? ((currentCell.maxBlockHistory.length > 2
            ? currentCell.maxBlockHistory[1]
            : currentCell.maxBlockHistory[0]) / 2)
...
function WfDescription(wf_adaptor, wf_illustrator) { // Model {{{
  ...
  function adjustColumnWidths() {
      ...
      // 块排序 用于之后minblock的分配 以及 包含关系
      blocks.sort((a, b) => {
        const singleColDiff = (a.col1 === a.col2) - (b.col1 === b.col2);
        if (singleColDiff !== 0) return singleColDiff; // 单列优先
      ...
      for (let i = blocks.length - 1; i >= 0; i--) {
        ...
        for (let col = col1; col <= col2; col++) {
          if (!columnWidths[col]) continue; // 跳过不存在的列
          
          if(Block.parallel && col == col1){
            continue;
          }
      ...
      if (columnWidths[1]) {
        ...
        for (let i = 1; i < Col1.length; i++) {
          if (Col1[i] && !Col1[i].alternative) {
            max1 = Math.max(max1, Col1[i].width || 0);
          }
        }
        columnWidths[1][0].width= max1;
    ...
    for (let i = blocks.length - 1; i >= 0; i--) {
      ...
      if (block.parallel&&block.col1 === 1) {
      
        let maxWidth = 0;
        console.log(block)
        // 遍历 row 找到最大 width
        for (let row = block.row1; row <= block.row2; row++) {
            if (columnWidths[1]?.[row]?.width !== undefined && !columnWidths[1]?.[row]?.alternative) {
                maxWidth = Math.max(maxWidth, columnWidths[1][row].width);
            }
        }
    
        // 如果 maxWidth 大于 minblock，则更新所有 minblock
        for (let row = block.row1; row <= block.row2; row++) {
            if (columnWidths[1]?.[row]?.minblock !== undefined) {
                if (maxWidth > columnWidths[1][row].minblock) {
                    columnWidths[1][row].minblock = maxWidth;
                }
            }
        }
      ...
      let maxContainedBy = columnWidths[block.col1]?.[block.row1]?.blocks[0];

      for (let row = block.row1; row <= block.row2; row++) {
        // 获取 blockmax / 2，如果 undefined 则跳过
      ...
      let maxblock = blocks[maxContainedBy];
      ...
      for (let row = block.row1; row <= block.row2; row++) {
        ...
        // 只在 block.col2 进行检查
        if (columnWidths[block.col2]?.[row]?.width >= columnWidths[block.col2]?.[row]?.block) {//看这个改变的block中最后一列是不是最长宽度
          //如果有，说明它的后面所有列都有可能要跟着它一起移动
          let foundMatchingBlock = false;//开始判断是否需要一起移动

          // 遍历 maxblock.row1 到 maxblock.row2 之间 block.col2+1 列的 symbol 为 true 的对象
          for (let r = maxblock.row1; r <= maxblock.row2; r++) {
            if (columnWidths[block.col2 + 1]?.[r]?.symbol === true) {
              // 遍历所有 block，检查这些 symbol=true 的点是否属于某个 block
              for (let j = 0; j < blocks.length; j++) {
                if (i === j) continue; // 跳过当前 block 自己
                const blockb = blocks[j];

                // 检查 symbol 所在的 (block.col2+1, r) 是否在 blockb 的范围内
                if (
                  blockb.col1 <= block.col2 + 1 && blockb.col2 >= block.col2 + 1 &&
                  blockb.row1 <= r && blockb.row2 >= r
                ) {
                  // 检查 blockb.col1 是否和 block.col1 相同
                  if (blockb.col1 === block.col1) {
                    foundMatchingBlock = true;
                    break; // 找到匹配的 block，跳出循环
                  }
                }
              }

              if (foundMatchingBlock) break; // 找到匹配的 block 就跳出
            }
          }
    ...
    for (const [blockIndex, changedCols] of changes.entries()) {
      ...
      if (!block) continue; // 避免 undefined 错误

      for (const col of changedCols) {
        let maxWidth = 0;

        // 在 block.row1 ~ block.row2 之间找最大 (width + offset)
        for (let row = block.row1; row <= block.row2; row++) {
          if (columnWidths[col]?.[row]) {
  ...
  var calculateSymbolGrid = function (root, parent_pos) {
    ...
    $(root).children().filter(function () { return this.localName[0] != '_'; }).each(function () {
      ...
      if (sname == "loop_head" || sname == "choose_inclusive" || sname == "choose_exclusive") {
        ...
        var width = getTextWidth($(context).attr('svg-label'), 'label');
